...
// ---------------------------------------------------------------------------
// Tiny SVG line/area chart (no dependencies)
// ---------------------------------------------------------------------------
function lineChart({ points, width = 320, height = 120, color = "#0ea5e9", markers = [], nowAt = null, fmtY = (v) => v }) {
  ...
  const path = points.map((p, i) => `${i ? "L" : "M"}${px(p.x.getTime()).toFixed(1)},${py(p.y).toFixed(1)}`).join(" ");
  ...
  // x-axis: a few day/time ticks
  const ticks = 4;
  for (let i = 0; i <= ticks; i++) {
    const t = minX + ((maxX - minX) * i) / ticks;
    const x = px(t);
    const lbl = new Date(t).toLocaleTimeString("en-US", { timeZone: TZ, hour: "numeric" });
